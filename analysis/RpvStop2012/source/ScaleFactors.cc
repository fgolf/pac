...
float TagAndProbeScaleFactor(int id, float pt, float eta)
{
    ...
    if (abs(id)==11) 
    {
        ...
        {
            cout << "[rpv_stop::TagAndProbeScaleFactor] WARNING -> eta is in the transition region. returning bogus value -999999" << endl;
            cout << Form("[rpv_stop::TagAndProbeScaleFactor] id: %d pt: %f eta: %f", id, pt, aeta) << endl; 
            return -999999.0;
        }
        ...
        cout << "[rpv_stop::TagAndProbeScaleFactor] WARNING -> missed coverage -- returning bogus value -999999" << endl;
        cout << Form("[rpv_stop::TagAndProbeScaleFactor] id: %d pt: %f eta: %f", id, pt, aeta) << endl; 
        return -999999.0;
    }
...
(
    ...
    const rpv_stop::AnalysisType::value_type& anal_type,
...
)
{
    ...
    if (anal_type == rpv_stop::AnalysisType::mmbb)
    {
...
std::pair<float, float> TTGammaScaleFactor(const unsigned int sr_num, const unsigned int analysis_type)
{
    ...
    if (analysis_type == 0)
    {
        ...
        switch (sr_num)
        {
            ...
            default : 
                      std::cout << Form("[rpv_stop::TTGammaScaleFactor] Signal Region %u not valid.  Return bogus value.", sr_num) << std::endl; 
    ...
    {
        std::cout << Form("[rpv_stop::TTGammaScaleFactor] AnalysisType %u not valid.  Return bogus value.", analysis_type) << std::endl; 
        return std::make_pair(-999999.0 , -999999.0); 
    }
} 
