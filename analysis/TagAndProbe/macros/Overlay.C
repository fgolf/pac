...
void Overlay(const std::string& filename = "plots/fits/SameSign_orig_v2/both/results.root", const std::string& suffix = "eps")
{
    ...
    for (size_t pt_bin = 0; pt_bin != npt_bins; pt_bin++)
    {
        ...
        p[plot_name]->SetYAxisRange(0, 1.2);
        ...
        p[plot_name]->Add(hc[Form("h_sf_vs_eta_pt%lu"      , pt_bin)], "data/MC", kGreen+2, 2, 24);
    ...
    for (size_t eta_bin = 0; eta_bin != neta_bins; eta_bin++)
    {
        ...
        p[plot_name]->Add(hc[Form("h_sf_vs_pt_eta%lu"      , eta_bin)], "data/MC", kGreen+2, 2, 24);
    }
