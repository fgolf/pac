...
(
    ...
    const ewkino::FakeRateType::value_type& fr_type,
...
{    
    m_fr_bin_info = ewkino::GetFakeRateBinInfo(m_fr_type);
    ...
    if (m_away_jet_pt < 0.)
    {
        if (m_fr_type == ewkino::FakeRateType::ss) m_away_jet_pt = 40.;
    }
    ...
    if (m_mu_iso_denom < 0.)
    {
        if (m_fr_type == ewkino::FakeRateType::ss) m_mu_iso_denom = 0.4;
    }
...
int FakeRateBabyLooper::operator()(long event, const std::string& current_file_name)
{
    ...
    try
    {
        ...
        if (is_el && is_qcd && (qcd_mu_low || qcd_mu_high))
        {
            return 0;
        }
        ...
        if (m_fr_type == ewkino::FakeRateType::ss)
        {
            if (is_data && is_mu)
            {
