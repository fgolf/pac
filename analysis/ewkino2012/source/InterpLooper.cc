...
(
    ...
    ewkino::AnalysisType::value_type analysis_type,
...
std::string GetSRLabel(ewkino::SignalRegion::value_type sr)
{
...
void InterpLooper::BookHists()
{
    try
    {
        ...
        {
            const string sr = GetSRLabel(static_cast<ewkino::SignalRegion::value_type>(m_sr_nums.at(i))); 
...
int InterpLooper::operator()(long event)
{
    ...
    try
    {
        ...
        for (size_t i = 0; i != m_sr_nums.size(); i++)
        {
            ...
            else
            {
                if (event == 0)
                {
                    ...
                    const float w = (rt::string_contains(ewkino_ss::dataset().Data(), "TTJets_FullLeptMGDecays_8TeV-madgraph") ? 1.0/(0.324*0.324) : 1.0);
        ...
        {
            if (m_verbose) {cout << "failing valid hypothesis requirement\t" << ewkino_ss::charge_type() << endl;}
            return 0;
        }
        ...
        for (size_t i = 0; i != m_sr_nums.size(); i++)
        {
            ewkino::SignalRegion::value_type signal_region = static_cast<ewkino::SignalRegion::value_type>(m_sr_nums.at(i));
            const string sr = GetSRLabel(signal_region); 

            if (is_ss()) 
            {
                // scaled numerator counts
                if (PassesSignalRegion(signal_region, m_analysis_type, m_signal_region_type, m_do_beff_sf, ewkino::SystematicType::None))
                {
                ...
                if (PassesSignalRegion(signal_region, m_analysis_type, m_signal_region_type, m_do_beff_sf, ewkino::SystematicType::JES_UP))
                {
                    rt::Fill2D(hc[sr+"nJESUP"], m0, m12, evt_weight);
                }

                if (PassesSignalRegion(signal_region, m_analysis_type, m_signal_region_type, m_do_beff_sf, ewkino::SystematicType::JES_DN))
                {
                    rt::Fill2D(hc[sr+"nJESDN"], m0, m12, evt_weight);
                }

                // JER scale up/scale down
                if (PassesSignalRegion(signal_region, m_analysis_type, m_signal_region_type, m_do_beff_sf, ewkino::SystematicType::JER))
                {
                    rt::Fill2D(hc[sr+"nJER"], m0, m12, evt_weight);
                }

                // Btag scale up/down
                if (PassesSignalRegion(signal_region, m_analysis_type, m_signal_region_type, m_do_beff_sf, ewkino::SystematicType::BEFF_UP, m_sample, m_is_fast_sim))
                {
                    rt::Fill2D(hc[sr+"nBTAUP"], m0, m12, evt_weight);
                }

                if (PassesSignalRegion(signal_region, m_analysis_type, m_signal_region_type, m_do_beff_sf, ewkino::SystematicType::BEFF_DN, m_sample, m_is_fast_sim))
                {
        ...
        for (unsigned int ix = 1; ix < nbinsx; ix++)
        {
            for (unsigned int iy = 1; iy < nbinsy; iy++)
            {
                ...
                if (den > 0)
                {
                    ...
                    const float stat_error = (m_use_cp_unc) ? (rt::GetClopperPearsonUncertainty(static_cast<int>(num), static_cast<int>(den))) / num : (num > 0 ? sqrt(eff*(1.0-eff)/den) : 0);
