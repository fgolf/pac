...
float TagAndProbeScaleFactor(int id, float pt, float eta)
{
    ...
    if (abs(id)==11) 
    {
        ...
        {
            std::cout << "[ewkino::TagAndProbeScaleFactor] WARNING -> eta is in the transition region. returning bogus value -999999" << std::endl;
            std::cout << Form("[ewkino::TagAndProbeScaleFactor] id: %d pt: %f eta: %f", id, pt, aeta) << std::endl; 
            return -999999.0;
        }
        ...
        std::cout << "[ewkino::TagAndProbeScaleFactor] WARNING -> missed coverage -- returning bogus value -999999" << std::endl;
        std::cout << Form("[ewkino::TagAndProbeScaleFactor] id: %d pt: %f eta: %f", id, pt, aeta) << std::endl; 
        return -999999.0;
    }
...
)
{
    ...
    if (anal_type == ewkino::AnalysisType::ss)
    {
...
std::pair<float, float> TTGammaScaleFactor(const unsigned int sr_num, const unsigned int analysis_type)
{
    ...
    {
        std::cout << Form("[ewkino::TTGammaScaleFactor] AnalysisType %u not valid.  Return bogus value.", analysis_type) << std::endl; 
        return std::make_pair(-999999.0 , -999999.0); 
    }
} 
