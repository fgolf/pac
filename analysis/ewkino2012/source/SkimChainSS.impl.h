        ...
        for (long event = 0; event != num_events_tree; ++event)
        {
            ...
            if (ewkino_ss::is_real_data())
            {
                ...
                if (!goodrun_file_name.empty())
                {
                    ...
                    if(!goodrun(ewkino_ss::run(), ewkino_ss::ls())) 
                    {
                        if (verbose) {cout << "Bad run and lumi:\t" << ewkino_ss::run() << ", " << ewkino_ss::ls() << endl;}
                        bad_events++;
                        continue;
                    }
                    //else
                ...
                {
                    at::DorkyEventIdentifier id = {ewkino_ss::run(), ewkino_ss::evt(), ewkino_ss::ls()};
                    if (is_duplicate(id))
                    {
                        if (verbose)
                        {
