...
try
{
    ...
    for (std::vector<std::string>::const_iterator vstring = v_mc_input_files.begin(); vstring != v_mc_input_files.end(); vstring++)
    {
        ...
        { 
            ...
            {
                cout << "[ewkino2012_ss_make_fr_from_hists]: MC input file " << *vstring << " not found" << endl;
                cout << desc << "\n";
                return 1;
            }
        }
    ...
    {
        cout << "[ewkino2012_ss_make_fr_from_hists]: no output file given.  printing to stdout" << endl;
    }

    // check mc files
    if (v_mc_input_files.empty())
    {
        cout << "[ewkino2012_ss_make_fr_from_hists]: no mc files given.  exiting" << endl;
        return 0;
    }
