...
void OverlayPlots(float scale = 3.95, int signal_region = 0, const std::string& suffix = "png")
{
    // samples to overlay
    ...
    {
        hcs.push_back(TH1Container(Form("plots/%s/%s.root", sr.c_str(), samples[i].name.c_str())));
        hcs[i].Scale(scale);
    }

    // overlay
    map<string, TH1Overlay> p;
    p["p_pt1_"   +sr] = TH1Overlay("higher p_{T} lepton;p_{T}^{lep1} (GeV)", "sb::off lg::top_right dt::stack");
    p["p_pt2_"   +sr] = TH1Overlay("lower p_{T} lepton;p_{T}^{lep2} (GeV)" , "sb::off lg::top_right dt::stack");
    p["p_mt_"    +sr] = TH1Overlay("m_{T};m_{T} (GeV)"                     , "sb::off lg::top_right dt::stack");
    p["p_ht_"    +sr] = TH1Overlay("H_{T};H_{T} (GeV)"                     , "sb::off lg::top_right dt::stack");
    p["p_met_"   +sr] = TH1Overlay("E_{T}^{miss};E_{T}^{miss} (GeV)"       , "sb::off lg::top_right dt::stack");
    p["p_njets_" +sr] = TH1Overlay("# jets;# jets"                         , "sb::off lg::top_right dt::stack");
    p["p_nbtags_"+sr] = TH1Overlay("# btags;# btags"                       , "sb::off lg::top_right dt::stack");
    ...
    for (size_t i = 0; i != hcs.size(); i++)
    {
        ...
        bool signal      = string_contains(name, "t1tttt");
        samples[i].yield = rt::Integral(hc["h_pt1"]);
        if (!signal)
        {
        ...
        // pt1
        p["p_pt1_"   +sr].Add(hc["h_pt1"   ], signal, title, color, 2, marker);
        p["p_pt2_"   +sr].Add(hc["h_pt2"   ], signal, title, color, 2, marker);
        p["p_mt_"    +sr].Add(hc["h_mt"    ], signal, title, color, 2, marker);
        p["p_ht_"    +sr].Add(hc["h_ht"    ], signal, title, color, 2, marker);
        p["p_met_"   +sr].Add(hc["h_met"   ], signal, title, color, 2, marker);
        p["p_njets_" +sr].Add(hc["h_njets" ], signal, title, color, 2, marker);
        p["p_nbtags_"+sr].Add(hc["h_nbtags"], signal, title, color, 2, marker);
    }
    ...
    // write
    Print(p, Form("plots/overlays/v2/sr%d", signal_region), suffix);
}
