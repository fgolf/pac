...
// construct:
FRClosureLooperETH::FRClosureLooperETH
(
...
// methods:
void FRClosureLooperETH::BeginJob()
{
    BookHists();
}

// end job
void FRClosureLooperETH::EndJob()
{
    // convenience alias
...
// book hists 
void FRClosureLooperETH::BookHists()
{
    try
    {
        // convenience alias
...
int FRClosureLooperETH::operator()(long event)
{
    using namespace std;
    using namespace ssb;
    ...
    try
    {
        ...
        if (is_sf())
        {
            ...
            if ((lep1_is_fo() && !lep1_is_num() && not_fromw_l1 && fromw_l2) || (lep2_is_fo() && !lep2_is_num() && fromw_l1 && not_fromw_l2))
            {
                if (is_sf() && mm()) ++nsf2;
        ...
        // printf("\nnumber of muon FOs: %4.2f", hc["h_sf_mufo_pt_vs_eta_mm"]->Integral());
    }
    catch (std::exception& e)
    {
        cout << e.what() << endl;
...
float FRClosureLooperETH::GetFakeRateValue(int lep_id, float pt, float eta) const 
{
    using namespace tas;
...
float FRClosureLooperETH::GetFakeRateError(int lep_id, float pt, float eta) const 
{
    using namespace tas;
