...
// FakeRatePrediction implementation
// ----------------------------------------------------//

// construct:
FakeRatePrediction::FakeRatePrediction(TH2* mufr, TH2* elfr, float scale)
    ...
    , m_mu_max(0)
{
    if (not h_mufr)
    {
        throw std::runtime_error("ERROR -- FakeRatePrediction - h_mufr is NULL!");
    }
    if (not h_elfr)
    {
        throw std::runtime_error("ERROR -- FakeRatePrediction - h_elfr is NULL!");
    }
...
)
{
    ...
    {
        throw std::runtime_error("ERROR - FakeRatePrediction::ComputeAllFakePredictions: one or more histograms are NULL");
    }
    // SF raw
