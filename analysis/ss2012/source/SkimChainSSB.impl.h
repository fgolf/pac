		...
		for (long event = 0; event != num_events_tree; ++event)
		{
			...
			if (ssb::is_real_data())
			{
				...
				if (!goodrun_file_name.empty())
				{
					...
					if(!goodrun(ssb::run(), ssb::ls())) 
					{
						if (verbose) {cout << "Bad run and lumi:\t" << ssb::run() << ", " << ssb::ls() << endl;}
						bad_events++;
						continue;
					}
					//else
				...
				{
					at::DorkyEventIdentifier id = {ssb::run(), ssb::evt(), ssb::ls()};
					if (is_duplicate(id))
					{
						if (verbose)
						{
