...
void EventInfoTree::FillCommon (Sample::value_type sample_, const std::string& root_file_name)
{
	...
	filt_csc       = cms2.evt_cscTightHaloId();
    filt_hbhe      = cms2.evt_hbheFilter();
    filt_hcallaser = cms2.filt_hcalLaser();
    filt_ecaltp    = cms2.filt_ecalTP();
    filt_trkfail   = cms2.filt_trackingFailure();
    filt_eebadsc   = (cms2.evt_isRealData() ? cms2.filt_eeBadSc() : true);
    filt_hbhenew   = passHBHEFilter();
...
void EventInfoTree::SetBranches(TTree &tree)
{
    ...
    tree.Branch(Form("%sfilt_hbhenew"   , prefix_.c_str()) , &filt_hbhenew   , "filt_hbhenew/O"   ); 
}
