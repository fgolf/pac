...
string TH1Overlay::ProcessOptionString(const std::string& option)
{
    ...
    for (size_t i = 0; i < DrawType_options.size(); i++) 
    {
        ...
        if (rt::string_contains(filtered_option, opt.second))
        {
            SetDrawType(opt.first);
            filtered_option = string_remove_all(filtered_option, opt.second);
        }
    }
    ...
    // axis logs
    if (rt::string_contains(filtered_option, "nology"))
    {
        SetLogy(false);
        filtered_option = string_remove_all(filtered_option, "nology");
    }
    if (rt::string_contains(filtered_option, "logy"))
    {
        SetLogy(true);
        filtered_option = string_remove_all(filtered_option, "logy");
    }
    if (rt::string_contains(filtered_option, "nologx"))
    {
        SetLogx(false);
        filtered_option = string_remove_all(filtered_option, "nologx");
    }
    if (rt::string_contains(filtered_option, "logx"))
    {
        SetLogx(true);
        filtered_option = string_remove_all(filtered_option, "logx");
    }
